...
<html lang="{{ config('app.locale') }}">
    ...
    <body style="background-color: #cecece;">
	 ...
	 <div class="login-box" style="position:relative">
		...
		<div>Please input your email to reset password</div>
		<form class="email-login" name="resetform" method="post">	
		  <div class="u-form-group">
			<input type="email" placeholder="Email" name="email" style="font-size:12px;" />
		  </div>
		  <div class="u-form-group">
			<a href="javascript:void(0)" onclick="resetProc(); return false;" class="btn btn-success">Reset</a>
			<a href="/member/login" onclick="window.location='/member/login';" class="btn btn-warning">Cancel</a>
		  </div>
		  {!! csrf_field() !!}
		</form>
	...
	<script>
		...
		var resetProc = function(){		
			...
			$(frm).append("<input type='hidden' name='_token' value='"+ $('input[name="_token"]').val()+"'>");
			frm.action = "/member/reset_password";
			frm.method = "POST";
			frm.submit();
		};
	</script>
